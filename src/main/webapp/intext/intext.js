/**
 * Created by xiaolongxu on 5/23/14.
 */

console.log("intext.js");
/**
 *  Override jQuery ajax call to be compatiable to IE 8 and IE9 for cross domain issue
 */
jQuery.post = (window.XDomainRequest) ? function(url, data, success, dataType) {
    console.log("Begin XDomainRequest. url="+url+", dataType="+dataType);
    var xdr = new XDomainRequest();
    xdr.timeout = 20000;
    xdr.open('post', url);
    xdr.onerror     = function() {console.log("XDomainRequest: Failed");};
    xdr.ontimeout   = function() {console.log("XDomainRequest: Timeout. timeout = " + xdr.timeout);};
    xdr.onload      = function() {console.log("XDomainRequest: Onload");};
    xdr.onprogress  = function() {console.log("XDomainRequest: Onprogress");};
    xdr.onload      = function() {
        console.log("XDomainRequest Success. response = "+xdr.responseText);
        var res = (dataType == "json") ? JSON.parse(xdr.responseText) : xdr.responseText;
        typeof success == "function" && success(res);
    };

    console.log("XDomainRequest data size="+data.length);
    var sendStatus = xdr.send(data);
} : jQuery.post; // else, use jQuery's method

/******************************** keywordpopup init **************************************/
var params       = Container.getParams(Container.intextId);
var paramsJson   = JSON.parse(decodeURIComponent(params));
IMWB_CCX_Ajax             = {};
IMWB_CCX_Ajax.ajaxurl     = (Container.isHttps() ? baseUrl : baseUrlHttp) + "/intext/getAd";
IMWB_CCX_Ajax.nonce       = "1234";
IMWB_CCX_Ajax.plugin_url  = (Container.isHttps() ? baseUrl : baseUrlHttp) + "/content/intext";
IMWB_CCX_Ajax.wp_root     = "";
IMWB_CCX_Ajax.sid         = paramsJson.sid;
IMWB_CCX_Ajax.said        = paramsJson.said;

/******************************** keywordpopup.js **************************************/
(function($) {
    var kwPopups = [];

    // make sure the popup position is updated on resize
    function handleWindowResize() {
        $.each(kwPopups, function() {
            this.refresh(true);
        });
    }
    $(window).resize(handleWindowResize);

    $.KeywordPopup = function(elm, options) {
        this.$elm = $(elm);
        this.opts = $.extend({}, $.fn.keywordpopup.defaults, options);
        this.$keywordPopup = $([
            '<div id="ccx-keywordpopup">',
                '<div class="ccx-keywordpopup-base">',
                    '<div class="ccx-keywordpopup-content">',
                        '<div class="ccx-keywordpopup-loading">Loading <i class="fa fa-spinner fa-spin fa-1x"></i></div>',
                    '</div>',
                    '<div class="ccx-footer">',
                        '<img src="'+IMWB_CCX_Ajax.plugin_url+'/images/logo.png">',
                        '<div class="ccx-actions">',
                            '<a href="'+IMWB_CCX_Ajax.plugin_url+'/help.html" target="_blank"><i class="fa fa-question ccx-question"></i></a>&nbsp;&nbsp;',
                            '<a href="javascript:jQuery.KeywordPopup.hideAll();"><i class="fa fa-times-circle ccx-close"></i></a>',
                        '</div>',
                    '</div>',
                '</div>',
            '</div>'].join('')).appendTo(document.body);

        this.$inner = this.$keywordPopup.find('div.ccx-keywordpopup-content');
        this.disabled = false;
        this.content = null;
        this.createTime = 0;
        this.init();
    ...
    $.KeywordPopup.hideAll = function() {
        $.each(kwPopups, function() {
            this.hide();
        });
    };

    $.KeywordPopup.prototype = {
        init: function() {
            kwPopups.push(this);

            // save a reference to the Poshytip object
            this.$elm.data('keywordpopup', this);

            // hook element events
            this.$elm.bind({
                'mouseenter.keywordpopup': $.proxy(this.mouseenter, this),
                'touchstart.keywordpopup': $.proxy(this.mouseenter, this)
//                'mouseleave.keywordpopup': $.proxy(this.mouseleave, this) // We ONLY remove popup when user click close button, by xxl
            });
            this.$keywordPopup.hover($.proxy(this.clearTimeouts, this)); // We ONLY remove popup when user click close button, by xxl
            this.$keywordPopup.bind('touchstart.keywordpopup', function(event){event.stopPropagation(); });
            $(document).bind({'touchstart.keywordpopup': $.proxy(this.mouseleave, this)});
        },
        mouseenter: function(e) {
            $.KeywordPopup.hideAll();
        ...
        mouseleave: function(e) {
            if (this.disabled || this.asyncAnimating && (this.$keywordPopup[0] === e.relatedTarget || jQuery.contains(this.$keywordPopup[0], e.relatedTarget)))
                return true;

            this.hideDelayed();
        },
        show: function() {
            if (this.disabled || this.$keywordPopup.data('active'))
                return;
        ...
        reset: function() {
            this.$keywordPopup.queue([]).detach().css('visibility', 'hidden').data('active', false);
            this.$inner.find('*').keywordpopup('hide');
            if (this.opts.fade)
                this.$keywordPopup.css('opacity', this.opacity);
        ...
        update: function() {
            ...
            // If item is more that 30 minutes old then re-fetch it
            if(this.content && ($.now() - this.createTime > 1800000)) {
                this.$inner.empty().append('<div class="ccx-keywordpopup-loading">Loading <i class="fa fa-spinner fa-spin fa-1x"></i></div>');
                this.content = null;
            }

            // We are using 'queryKeyword' as keyword, by xxl
            var bid              = this.$elm.data('bid');
            var highlightKeyword = this.$elm.data('keyword');
            var queryKeyword     = this.$elm.data('queryKeyword');
            var sid              = IMWB_CCX_Ajax.sid;
            var said             = IMWB_CCX_Ajax.said;
            var ajaxurl          = IMWB_CCX_Ajax.ajaxurl;

            // Every time user mouseover, get ad for them, no cache. by xxl.
            var self = this;
            var data = { action :'get_contextual_ad', bid: bid, highlightKeyword: highlightKeyword, queryKeyword: queryKeyword, sid: sid, said: said};
            $.post(ajaxurl,
                JSON.stringify(data),
                function(html) {
                    if (html) {
                        self.$inner.empty().append(html);
                        self.content = html;
                        self.createTime = $.now();
                        self.refresh(true);
                    } else {
        ...
        refresh: function(async) {
            ...
            if (async) {
                if (!this.$keywordPopup.data('active'))
                    return;
                // save current position as we will need to animate
                var currPos = {left: this.$keywordPopup.css('left'), top: this.$keywordPopup.css('top')};
            }

            // reset position to avoid text wrapping, etc.
            this.$keywordPopup.css({left: 0, top: 0}).appendTo(document.body);

            // save default opacity
            if (this.opacity === undefined)
                this.opacity = this.$keywordPopup.css('opacity');

            this.kwPopupOuterW = this.$keywordPopup.outerWidth();
            this.kwPopupOuterH = this.$keywordPopup.outerHeight();
            ...
            if (async && this.opts.refreshAniDuration) {
                ...
                var self = this;
                this.$keywordPopup.css(currPos).animate({left: this.pos.l, top: this.pos.t}, this.opts.refreshAniDuration, function() { self.asyncAnimating = false; });
            } else {
                this.$keywordPopup.css({left: this.pos.l, top: this.pos.t});
            }
        },
        display: function(hide) {
            var active = this.$keywordPopup.data('active');
            if (active && !hide || !active && hide)
                return;

            this.$keywordPopup.stop();
            if (this.opts.fade && (hide && this.opts.hideAniDuration || !hide && this.opts.showAniDuration)) {
                var from = {}, to = {};
                if (this.opts.fade) {
                    from.opacity = hide ? this.$keywordPopup.css('opacity') : 0;
                    to.opacity = hide ? 0 : this.opacity;
                }
                this.$keywordPopup.css(from).animate(to, this.opts[hide ? 'hideAniDuration' : 'showAniDuration']);
            }
            hide ? this.$keywordPopup.queue($.proxy(this.reset, this)) : this.$keywordPopup.css('visibility', 'inherit');
            this.$keywordPopup.data('active', !active);
        },
        disable: function() {
            this.reset();
            this.disabled = true;
        ...
        destroy: function() {
            this.reset();
            this.$keywordPopup.remove();
            delete this.$keywordPopup;
            this.content = null;
            this.$elm.unbind('.keywordpopup').removeData('keywordpopup');
            kwPopups.splice($.inArray(this, kwPopups), 1);
        },
        ...
        calcPos: function() {
            ...
            // keep in viewport - Horizontal
            pos.l = xL;
            if (this.opts.keepInViewport && pos.l < win.l)
                pos.l = win.l+this.opts.viewPortMargin;
            if (this.opts.keepInViewport && pos.l+this.kwPopupOuterW > win.w)
                pos.l = pos.l-((pos.l+this.kwPopupOuterW )-win.w)-this.opts.viewPortMargin;

            // Keep in viewport - Vertical
            pos.t = yT-this.kwPopupOuterH;
    ...
    $.fn.keywordpopup = function(options) {
        if (typeof options == 'string') {
            ...
            if (method == 'destroy') {
                this.die ?
                    this.die('mouseenter.keywordpopup').die('focus.keywordpopup') :
                    $(document).undelegate(this.selector, 'mouseenter.keywordpopup').undelegate(this.selector, 'focus.keywordpopup');
            }
            return this.each(function() {
                var keywordpopup = $(this).data('keywordpopup');
                if (keywordpopup && keywordpopup[method])
                    keywordpopup[method].apply(keywordpopup, args);
            });
        }

        var opts = $.extend({}, $.fn.keywordpopup.defaults, options);
        ...
        if (opts.liveEvents) {
            ...
            handler = function() {
                var $this = $(this);
                if (!$this.data('keywordpopup'))
                    $this.keywordpopup(deadOpts).keywordpopup('mouseenter');
            };
            // support 1.4.2+ & 1.9+
            this.live ?
                this.live('mouseenter.keywordpopup', handler) :
                $(document).delegate(this.selector, 'mouseenter.keywordpopup', handler);
            return this;
        }

        return this.each(function() {
            new $.KeywordPopup(this, opts);
        });
    };

    // default settings
    $.fn.keywordpopup.defaults = {
        ...
        viewPortMargin:   10,      // Margin to use when adjusting popup to fit in viewport, so it doesnt butt up against the browser edge
        fade:            true,		// use fade animation
        slide:            true,		// use slide animation
        slideOffset:      8,		// slide animation offset
        showAniDuration:   500,		// show animation duration - set to 0 if you don't want show animation
...
//  =======
        ...
        $D.start = function ()
        {
            ...
            /** Comment Intended as we don't need article title, by xxl
             *
             *
...
//  start }
        ...
        return $D; }
    /******************************** Highlight Logic **************************************/
    var adKeywordHighlighted = [];

    jQuery.fn.highlight = function(keywordInfo) {
        var keywordWithRegex = keywordInfo.highlightKeyword;
        var queryKeyword     = keywordInfo.queryKeyword;
        var contextWithRegex = keywordInfo.context;
        var bid              = keywordInfo.bid;

        if(jQuery.inArray(keywordWithRegex.toLowerCase(),adKeywordHighlighted) != -1) {
            return;
        }

        var contextRegex = new RegExp("\\b"+ contextWithRegex + "\\b");
        var keywordRegex = new RegExp("\\b("+ keywordWithRegex + ")\\b");

        function innerHighlight(node) {
            var skip = 0;
            if (node.nodeType === 3 ) { // 3 - Text node
                var text = node.data;
                var contextPos = text.search(contextRegex);
                var realKeyword = keywordRegex.exec(text) ? keywordRegex.exec(text)[1] : null;

                if (contextPos < 0 || text.length == 0 || !realKeyword || $(node.parentNode).hasClass('ccx-highlight')) {
                    return 0;
                }

                var pos = contextPos + text.substring(contextPos).search(keywordRegex);

                var spanNode = document.createElement('span');
                spanNode.className = 'ccx-highlight'; // set css

                var middleBit = node.splitText(pos); // split to 2 nodes, node contains the pre-pos text, middleBit has the post-pos
                var endBit = middleBit.splitText(realKeyword.length); // similarly split middleBit to 2 nodes
                var middleClone = middleBit.cloneNode(true);
                spanNode.appendChild(middleClone);
                // parentNode ie. node, now has 3 nodes by 2 splitText()s, replace the middle with the highlighted spanNode:
                middleBit.parentNode.replaceChild(spanNode, middleBit);
                $(spanNode).data('bid',bid);
                $(spanNode).data('keyword',realKeyword);
                $(spanNode).data('queryKeyword',queryKeyword);

                skip = 1; // skip this middleBit, but still need to check endBit
                adKeywordHighlighted.push(keywordWithRegex.toLowerCase());
                console.log("Highlight: Highlighted keyword: "+realKeyword);
            } else if (node.nodeType === 1 && node.childNodes && !/(script|style)/i.test(node.tagName)) { // 1 - Element node
                for (var i = 0; i < node.childNodes.length; i++) { // highlight all children
                    i += innerHighlight(node.childNodes[i]); // skip ccx-highlighted ones
                }
            }
            return skip;
        }
    ...
    jQuery.fn.removeHighlight = function() {
        adKeywordHighlighted = [];
        return this.find("span.ccx-highlight").each(function() {
//        this.parentNode.firstChild.nodeName;
    ...
    /*
     * Base on 'Clearly' detect result, extract the main part
     * */
    var bulkTextExtractor = function(strHTML, threshold) {
        var $ = jQuery;
        var countWords = function (str) {return str ? str.split(/\s+/).length : 0;};
        var sel = "div,p";
        var srcElement = document.createElement("div");
        srcElement.innerHTML = strHTML;
        var maxLengthElement = null;
        var maxElementWordCount = 0;

        var sumPLeafNodes = 0;
        var sumDivLeafNodes = 0;

        $(srcElement).find(sel).each(function() {
            if ($(this).children().length == 0 && $(this).text() > '') {
                var blockWordCount = countWords($(this).text());
                if (this.tagName == "DIV") {
                    sumDivLeafNodes +=  blockWordCount
                } else {
                    sumPLeafNodes += blockWordCount
                }

                if (blockWordCount > maxElementWordCount && blockWordCount > threshold) {
                    maxElementWordCount = blockWordCount;
                    maxLengthElement = this;
                }
            }
        });

        if (maxLengthElement == null) return "";

        var sumOfTextBlock = maxLengthElement.tagName == "DIV" ? sumDivLeafNodes : sumPLeafNodes;
        var parents = $(maxLengthElement).parents().toArray();
        for (var i=0;i<parents.length;i++) {
            var p = parents[i];
            if (p.tagName == "DIV" && countWords($(p).text()) *100/sumOfTextBlock >= 50) {
                return $(p).html();
            }
        }
    };
...
// Callback function of detect
    var getKwList = function (detectedContent) {
        alert("detect finish.");
        var $ = jQuery;
        console.log("intext.js: detect finish.");
        var wordDistance    = paramsJson.intextWordDistance;
        var adLimit         = paramsJson.intextAdLimit;
        var sid             = paramsJson.sid;
        var said            = paramsJson.said;
        var url             = (Container.isHttps() ? baseUrl : baseUrlHttp) + "/intext/findKeywords?adLimit=" + adLimit + "&wordDistance=" + wordDistance + "&sid=" + sid + "&said=" + said;
        var mainContent     = bulkTextExtractor(detectedContent._html, 40);

        if (mainContent == "") {
            console.log("intext.js: getKwList(): no content to highlight.");
            return;
        }

        console.log("intext.js: getKwList(): mainContent char size =" + mainContent.length);
        $.post(url,
            mainContent,
            function (res) {
                console.log("intext.js: getKwList(): response = "+res);
                var jsonKwList = res;
                $('body').removeHighlight();
                $.each(jsonKwList, function(index, keywordInfo) {
                    if (keywordInfo.highlightKeyword) {
                        $('body').highlight(keywordInfo);
                    } else {
                        console.log("intext.js: getKwList(): No keyword returned.");
                    }
                });


                $('body').find('.ccx-highlight').keywordpopup();
            },
            "json");
    };
...
// call -- returns nothing; callbacks will be used
    alert("intext.js start");
    console.log("intext.js: detect start.");
    _detect.start();
// test websites:
// http://www.cnn.com/2014/05/13/travel/malaysia-airlines-flight-tracking/index.html?iid=article_sidebar
